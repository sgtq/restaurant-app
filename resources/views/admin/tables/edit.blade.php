            ...
            <div class="m-2 p-2 bg-slate-100 rounded">
                <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                    <form method="POST" action="{{ route('admin.tables.update', $table->id) }}">
                        ...
                        <div class="sm:col-span-6">
                            <label for="guest_number" class="block text-sm font-medium text-gray-700"> Seats </label>
                            <div class="mt-1">
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <select id="status" name="status"
                                    class="form-select block w-full mt-1 @error('status') border-red-400 @enderror">
                                @foreach ($tableStatuses as $status)
                                    <option value="{{ $status->id }}" @selected($table->status->id == $status->id)>
                                        {{ $status->name }}
                                    </option>
                                @endforeach;
                            </select>
                        ...
                        <div class="sm:col-span-6">
                            <label for="tableLocation" class="block text-sm font-medium text-gray-700"> Location </label>
                            <select id="location" name="location"
                                    class="form-select block w-full mt-1 @error('location') border-red-400 @enderror">
                                @foreach ($tableLocations as $location)
                                <option value="{{ $location->id }}" @selected($table->location->id == $location->id )>
                                 {{ $location->name }}
                                </option>
                                @endforeach;
                            </select>
